...
class NumbersScreen extends StatefulWidget {
  ...
  NumbersScreen({required this.imagePaths});
...
class _NumbersScreenState extends State<NumbersScreen> {
  ...
  Future<void> _speakCurrentWord() async {
    ...
    String number = _extractNumberFromImagePath(imagePath);
    print("Speaking: $number $recognizedText");
    await flutterTts.speak("$number $recognizedText");
  }

  String _extractNumberFromImagePath(String imagePath) {
    // Extract the number from the image path, assuming the format is assets/learn_numbers/1.png
    String number = imagePath.split('/').last.split('.').first;
...
void main() => runApp(MaterialApp(home: NumbersScreen(imagePaths: List.generate(10, (index) => 'assets/learn_numbers/${index == 9 ? 0 : index + 1}.png'))));
